...
// Copy constructor
{
    setNode(n.getId(), n.getWeight(), n.isMarked(), n.alrVisited());
}
...
ostream& operator<<(ostream& ostr, const node& n)
{
    ostr << "node: " << n.getId() << " weight: " << n.getWeight()
        << " visited: " << n.alrVisited() << " marked " << n.isMarked() << endl;
...
// Print all edge information for a valid edge;
{
    ...
    cout << " weight: " << e.getWeight() << " visited: " << e.alrVisited()
...
public:
    ...
    bool Edge(NodeType i, NodeType j) const;
    int numNodes() const;
    int numEdges() const;
    ...
    bool alrVisited(int i, int j) const;
    bool alrVisited(int i) const;
    void clearVisit();
...
bool graph::Edge(NodeType i, NodeType j) const
...
bool graph::alrVisited(int i) const
...
// exception if i is too large or too small.
{
    ...
    return nodes[i].alrVisited();
}

bool graph::alrVisited(int i, int j) const
